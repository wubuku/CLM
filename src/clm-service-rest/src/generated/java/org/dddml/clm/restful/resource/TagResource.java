...
public class TagResource {
    ...
    public String post(@RequestBody CreateOrMergePatchTagDto.CreateTagDto value,  HttpServletResponse response) {
        try {
            TagCommand.CreateTag cmd = value;//.toCreateTag();
            cmd.setRequesterId(SecurityContextUtil.getRequesterId());
            String idObj = tagApplicationService.createWithoutId(cmd);
     ...
     */
    @PutMapping("{tagId}")
    public void put(@PathVariable("tagId") String tagId, @RequestBody CreateOrMergePatchTagDto value) {
        try {
            if (value.getVersion() != null) {
                value.setCommandType(Command.COMMAND_TYPE_MERGE_PATCH);
                TagCommand.MergePatchTag cmd = (TagCommand.MergePatchTag) value.toSubclass();
                TagResourceUtils.setNullIdOrThrowOnInconsistentIds(tagId, cmd);
                cmd.setRequesterId(SecurityContextUtil.getRequesterId());
            ...
            TagCommand.CreateTag cmd = (TagCommand.CreateTag) value.toSubclass();
            TagResourceUtils.setNullIdOrThrowOnInconsistentIds(tagId, cmd);
            cmd.setRequesterId(SecurityContextUtil.getRequesterId());
            tagApplicationService.when(cmd);
     ...
     */
    @PatchMapping("{tagId}")
    public void patch(@PathVariable("tagId") String tagId, @RequestBody CreateOrMergePatchTagDto.MergePatchTagDto value) {
        try {

            TagCommand.MergePatchTag cmd = value;//.toMergePatchTag();
            TagResourceUtils.setNullIdOrThrowOnInconsistentIds(tagId, cmd);
            cmd.setRequesterId(SecurityContextUtil.getRequesterId());
            tagApplicationService.when(cmd);
     ...
     */
    @DeleteMapping("{tagId}")
    public void delete(@PathVariable("tagId") String tagId,
                       @NotNull @RequestParam(value = "commandId", required = false) String commandId,
                       @NotNull @RequestParam(value = "version", required = false) @Min(value = -1) Long version,
                       @RequestParam(value = "requesterId", required = false) String requesterId) {
        try {

            TagCommand.DeleteTag deleteCmd = new DeleteTagDto();

            deleteCmd.setCommandId(commandId);
            deleteCmd.setRequesterId(requesterId);
            deleteCmd.setVersion(version);
            TagResourceUtils.setNullIdOrThrowOnInconsistentIds(tagId, deleteCmd);
            deleteCmd.setRequesterId(SecurityContextUtil.getRequesterId());
            tagApplicationService.when(deleteCmd);
